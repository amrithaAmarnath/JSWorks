var countries=[
  ...
  //flag,countryname,population,capital,region
 function displayCountries(countries){
  ...
  for (let c of countries){
    ...
    <div class="card" style="width: 100%;">
        <img src="${c.flags.png}" class="card-img-top" alt="..." height="300px";>
        <div class="card-body">
          <h5 class="card-title">Name: <span>${c.name}</span></h5>
          <p class="card-text">Population: <span>${c.population}</span></p>
        </div>
        <ul class="list-group list-group-flush">
          <li class="list-group-item">Capital: <span>${c.capital}</span></li>
          <li class="list-group-item">Region: <span>${c.region}</span></li>
          <li class="list-group-item">Subregion: <span>${c.subregion}</span></li>
        </ul>
  ...
  var regions= new Set();
  countries.forEach(c=>regions.add(c.region))

  console.log(regions)
var regionset=new Set()

  countries.map(c=>{regionset.add(c.region)
  ...
  function displayRegions(regions){
    ...
    for(let reg of regions){
      data+=`<div>
        <button class="btn btn-outline-info countryname" onclick="filterCountriesByRegion('${reg}')" value=${reg}>${reg}</button>
      </div>`
    }
    document.querySelector('#id_regions').innerHTML=data
  }
  ...
  function displaycountry(){
   ...
   //    let country=countries.filter(c=>c.name==cname)
      // displayCountries(country);
      let country=countries.find(c=>c.name==cname)
      console.log(countries.find(c=>c.name==cname))
      let htmlData = `
      <div class="col-4 mt-4">
      <div class="card" style="width: 100%;">
          <img src="${country.flags.png}" class="card-img-top" alt="..." height="300px";>
          <div class="card-body">
            <h5 class="card-title">Name: <span>${country.name}</span></h5>
            <p class="card-text">Population: <span>${country.population}</span></p>
          </div>
          <ul class="list-group list-group-flush">
            <li class="list-group-item">Capital: <span>${country.capital}</span></li>
            <li class="list-group-item">Region: <span>${country.region}</span></li>
            <li class="list-group-item">Subregion: <span>${country.subregion}</span></li>
          </ul>
